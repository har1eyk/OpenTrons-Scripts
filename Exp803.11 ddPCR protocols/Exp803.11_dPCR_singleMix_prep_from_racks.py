...
# metadata
metadata = {
    'protocolName': 'Uses a Single Mix and Racks to Add to 2 Qiagen 24w, 26k NanoPlate.',
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    rack_one = protocol.load_labware('vwr_24_tuberack_1500ul', '1')
    rack_two = protocol.load_labware('vwr_24_tuberack_1500ul', '4')
    ...
    for col in range(1, 4):
        for row in 'ABCDEFGH':
            well = row+str(col)
            p300.pick_up_tip()
            p300.aspirate(22, deepwell_wells[pos])
