...
# metadata
metadata = {
    'protocolName': 'Tip Touch for Qiagen Nanoplate 24w, 26k.',
    'author': 'Harley King <harley.king@luminultra.com>',
    'description': 'Testing the tip touch on Qiagen custom labware.',
    'apiLevel': '2.12'
}
##########################
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    nano_plate = protocol.load_labware('qiagen_24_wellplate_60ul', '5')
    ...
    p300.pick_up_tip()
    p300.move_to(nano_plate['A1'].top(5)) # move to top of plate
    protocol.delay(seconds=5, minutes=0)
    p300.move_to(nano_plate['A1'].center()) # move to top of tube
    protocol.delay(seconds=5, minutes=0)
    p300.move_to(nano_plate['A1'].bottom(2)) # move to top of tube
    protocol.delay(seconds=5, minutes=0)
    p300.touch_tip(nano_plate['A1'], v_offset=-1, speed=20.0)
    
    p300.move_to(nano_plate['H3'].top(5)) # move to top of plate
    protocol.delay(seconds=5, minutes=0)
    p300.move_to(nano_plate['H3'].center()) # move to top of tube
    protocol.delay(seconds=5, minutes=0)
    p300.move_to(nano_plate['H3'].bottom(2)) # move to top of tube
    protocol.delay(seconds=5, minutes=0)
    p300.touch_tip(nano_plate['H3'], v_offset=-1, speed=20.0)
