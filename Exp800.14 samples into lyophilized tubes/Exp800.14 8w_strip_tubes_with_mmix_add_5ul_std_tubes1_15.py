...
# metadata
metadata = {
    'protocolName': 'Mastermix in 8-well strips from Tubes 1 to 15 plus NTC. Adds 5ul.',
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    num_of_sample_reps = 4 # doesn't yet accommodate other ints
    ...
    for i, (stdtube, row) in enumerate(zip(all_stds, dest_rows)): #loop through all std tubes
        # p300.pick_up_tip()
        p20.pick_up_tip()
        p20.move_to(stdtube.bottom(40))
        p20.aspirate(5*num_of_sample_reps, stdtube)
        protocol.delay(seconds=1) #equilibrate
        p20.touch_tip(v_offset=-12) # two levels to remove residue
        protocol.delay(seconds=1) #equilibrate
        p20.touch_tip(v_offset=-4) # remove residue 
        start = 0
        ...
        for y in range(start, stop):
            ...
            col = 2*y+1
            dest = row+str(col)
            p20.move_to(holder[dest].bottom(40)) # move above caps
            p20.dispense(tot_sample_vol, holder[dest].bottom(1))
            protocol.delay(seconds=1)
            p20.touch_tip(v_offset=-12) #touch tip x mm below the top of the current location
            p20.move_to(holder[dest].top()) # need this move to recenter tip so it doesn't drag up tubes
            p20.move_to(holder[dest].bottom(40)) # position above caps
        p20.move_to(holder[dest].bottom(40)) # position above caps
        p20.drop_tip()