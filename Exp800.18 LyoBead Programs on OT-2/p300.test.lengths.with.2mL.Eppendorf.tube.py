...
# metadata
metadata = {
    'protocolName': 'Testing values and lengths in Eppendorf 2mL tube on alum block.',
    'author': 'Harley King <harley.king@luminultra.com>',
    'description': 'Testing top and bottom functions with Epp 2mL tube on alum block and in rack.',
    'apiLevel': '2.12'
}
##########################
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    # cont_pos1 = protocol.load_labware('vwr_24_tuberack_1500ul', '3')
    ...
    # paraFilm = protocol.load_labware('opentrons_96_aluminumblock_generic_pcr_strip_200ul', '1')
    # paraFilm = protocol.load_module('nest_96_wellplate_200ul_flat', '1')
    ...
    # mmixVol = 1350 # if in 2mL tube, how much mastermix volume
    # dispVol = 14.8
    ...
    p300.pick_up_tip()
    p300.move_to(alumBlock['D1'].top())
    protocol.delay(seconds=5)
    p300.move_to(alumBlock['D1'].bottom(5))
    protocol.delay(seconds=5)
    p300.drop_tip() 
    # p300.move_to(alumBlock['D6'].top())
    # protocol.delay(seconds=5)
    # p300.move_to(alumBlock['D6'].bottom())
    # protocol.delay(seconds=5)
    
    
    # contH = 20 # how many mm above tuberack height is the container height? 110mm is max.
    # # mmixH = fifty_ml_heights(17500, 20, 200) 
    # mmixH = tip_heightsEpp(mmixVol, int(totalPipetteRefills), dispVol*4) #mix volume, total steps = totalPipetteRefills in dispVol*4 pipette volume  
    ...
    # for i, row in enumerate(rows): # two loops because 8 dispenses
    ...
    #         dest = paraFilm[row+str(dispNo+1)]
