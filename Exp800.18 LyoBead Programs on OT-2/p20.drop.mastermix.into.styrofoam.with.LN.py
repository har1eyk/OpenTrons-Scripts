...
# metadata
metadata = {
    'protocolName': 'LyoBead Creation on OT-2 in Styrofoam Container with LN using P20.',
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    p20.pick_up_tip()
    contH = 107 # how many mm above tuberack height is the container height? 110mm is max for p300, 108 for p20
    # mmixH = fifty_ml_heights(17500, 20, 200) 
    mmixH = tip_heights(mmixVol, int(totalPipetteRefills), dispVol) #mix volume, total steps = totalPipetteRefills in dispVol*10 pipette volume  
    # prewetting step for tip
    p20.mix(3, 20, mmix.bottom(mmixH[0])) #only need 9 dispenses at this volume: 1 is for burn, next 8 drop in container, last is for accuracy.
    for i in range(int(totalPipetteRefills)):  
        # tip preparation
        p20.aspirate(dispVol, mmix.bottom(mmixH[i]))
        p20.move_to(mmix.bottom(mmixH[i]+20))
        p20.dispense(dispVol, mmix.bottom(mmixH[i]+20)) # dispense dispVol to improve volume accuracy in subsequent dispenses
        protocol.delay(seconds=4) # tip for drops to coalesce
        # tip dispensing
        for dispNo in range(4):
            p20.aspirate(dispVol, mmix.bottom(mmixH[i]))
            protocol.delay(seconds=2) #equilibrate
            p20.move_to(mmix.bottom(mmixH[i]+10)) # droplets coalesce
            protocol.delay(seconds=2)
            p20.move_to(mmix.bottom(mmixH[i])) # touch tip to remove droplets
            dest = dispPos[dispNo]
            p20.move_to(dest.bottom(contH)) # move to destination and pause for a few seconds to remove lateral motion
            protocol.delay(seconds=2)
            p20.dispense(dispVol, dest.bottom(contH), rate = 0.1) # want height to above parafilm, but not too high
            p20.move_to(dest.bottom(contH+1))
            p20.move_to(dest.bottom(contH))
            p20.move_to(mmix.bottom(mmixH[i]+10)) # drop waste mix back into tube
    p20.drop_tip()

#20220915 Droplet breaks off into 10ul. This vol isn't enough to include all reagents.
