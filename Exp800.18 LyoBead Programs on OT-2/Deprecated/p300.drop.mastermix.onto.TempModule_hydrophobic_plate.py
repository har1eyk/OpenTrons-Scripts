...
# metadata
metadata = {
    'protocolName': 'LyoBead Creation on OT-2 Deposited on 96w Hydrophobic Plate on Temp Module.',
    'author': 'Harley King <harley.king@luminultra.com>',
    'description': 'Drops 14.5ul mastermix dispenses into 96w hydrophobic plate chilled on the temp module, no spacer.',
    'apiLevel': '2.12'
}
##########################
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    # plate = tempdeck.load_labware('96whydrophobicplateontempmodule_96_wellplate_200ul')
    plate = tempdeck.load_labware('noah_hydrophobic_thin_96_wellplate_200ul')
    ...
    # mmix = rack['D1'] # if in 2mL tube, what position on aluminum block
    mmixVol = 1000 # if in 2mL tube, how much mastermix volume
    dispVol = 14.5
    ...
    p300.pick_up_tip()
    p300.move_to(plate['A1'].center())
    protocol.delay(seconds = 5)
    p300.move_to(plate['A1'].bottom())
    protocol.delay(seconds = 5)
    p300.move_to(plate['H12'].center())
    protocol.delay(seconds = 5)
    p300.move_to(plate['H12'].bottom())
    protocol.delay(seconds = 5)
