# imports
from re import I
from opentrons import protocol_api

# metadata
metadata = {
    'protocolName': 'Distribute 14ul ATP Mastermix and Add Dextran and Water to 20ul in Corning Plate.',
    'author': 'Harley King <harley.king@luminultra.com>',
    'description': 'Testing Dextran Concentrations in ATP Mastermix.',
    'apiLevel': '2.13'
}

##########################
def tip_heightsEpp(init_vol, steps, vol_dec):
    vols = []
    heights = []
    # these values originate from Excel spreadsheet "Exp803..."
    p0=-0.272820744
    p1=0.019767959
    p2=2.00442E-06
    p3=-8.99691E-09
    p4=6.72776E-12
    p5=-1.55428E-15
    if init_vol > 2000:
        offset = 12 # model out of range; see sheet
    else:
        offset = 11 #mm Need to add offset to ensure tip reaches below liquid level
    for i in range(steps):
        x = init_vol-vol_dec*i
        vols.append(x)
        h = p5*x**5+p4*x**4+p3*x**3+p2*x**2+p1*x**1 + p0
        h = h-offset
        if h < 7: # prevent negative heights; go to bottom to avoid air aspirant above certain height
            h = 1        
            heights.append(h)
        else:
            heights.append(round(h, 1))
    return heights

def tip_heights(init_vol, steps, vol_dec):
    vols = []
    heights = []
    # these values originate from Excel spreadsheet "Exp803..."
    p0=0.029502064
    p1=0.084625954
    p2=-0.000174864
    p3=2.18373E-07
    p4=-1.30599E-10
    p5=2.97839E-14
    if init_vol > 1500:
        offset = 14 # model out of range; see sheet
    else:
        offset = 5 #mm Need to add offset to ensure tip reaches below liquid level
    for i in range(steps):
        x = init_vol-vol_dec*i
        vols.append(x)
        h = p5*x**5+p4*x**4+p3*x**3+p2*x**2+p1*x**1 + p0
        h = h-offset
        if h < 7: # prevent negative heights; go to bottom to avoid air aspirant above certain height
            h = 1        
            heights.append(h)
        else:
            heights.append(round(h, 1))
    return heights

# Calc heights for 50mL conical tubes
def fifty_ml_heights(init_vol, steps, vol_dec):
    vols = []
    heights = []
    # these values originate from Excel spreadsheet "Exp803..."
    b = 0
    m = 0.0024
    if init_vol > 51000:
        offset = 14  # model out of range; see sheet
    else:
        offset = 15  # mm Need to add offset to ensure tip reaches below liquid level
    for i in range(steps):
        x = init_vol-vol_dec*i
        vols.append(x)
        h = m*x+b
        h = h-offset
        if h < 17.5:  # If less than 5mL remain in 50mL tube, go to bottom for asp
            h = 2
            heights.append(h)
        else:
            heights.append(round(h, 1))
    return heights

def run(protocol: protocol_api.ProtocolContext):

    # EXPERIMENT OUTLINE
    # This experiment tests if dextran or PEG concentrations in the ATP Mastermix affect the luminescent results.
    # Dextran is in 50mL tube
    # Mastermix is added to 2mL tube. EAch column is a concentration of Dextran. 12 different concentrations.
    # To these tubes dextran and water are added, mixed and aliquoted to PCR plate.

    
    # LABWARE
    tiprack300 = protocol.load_labware('opentrons_96_filtertiprack_200ul', '8')
    tiprack20 = protocol.load_labware('opentrons_96_filtertiprack_20ul', '9')
    
    plate = protocol.load_labware('corning_96_wellplate_360ul_flat', '1')
    
    mix_rack = protocol.load_labware('opentrons_24_tuberack_nest_1.5ml_snapcap', '2')
    reagent_rack = protocol.load_labware('opentrons_24_tuberack_nest_1.5ml_snapcap', '3')
    mastermix_rack = protocol.load_labware('eppendorf_24_tuberack_2000ul', '4') # 2mL Eppendorf tube with mastermix


    # reagent_rack = protocol.load_labware('opentrons_10_tuberack_nest_4x50ml_6x15ml_conical', '2')

    # PIPETTES
    p20 = protocol.load_instrument(
        'p20_single_gen2', 'right', tip_racks=[tiprack20]
    )

    p300 = protocol.load_instrument(
        'p300_single_gen2', 'left', tip_racks=[tiprack300]
    )
    
    # REAGENTS
    # mix_rack   
    cond_1 = mix_rack['A1']  # empty, "conditions" for each column
    cond_2 = mix_rack['A2']  # empty
    cond_3 = mix_rack['A3']  # empty
    cond_4 = mix_rack['A4']  # empty
    cond_5 = mix_rack['A5']  # empty
    cond_6 = mix_rack['A6']  # empty
    cond_7 = mix_rack['B1']  # empty
    cond_8 = mix_rack['B2']  # empty
    cond_9 = mix_rack['B3']  # empty
    cond_10 = mix_rack['B4']  # empty
    cond_11 = mix_rack['B5']  # empty
    cond_12 = mix_rack['B6']  # empty
    # reagent_rack          
    water = reagent_rack['A6'] # 1.5mL tube with water, 1000uL
    dextran = reagent_rack['A1'] # 1.5mL tube with dextran, 1000uL Sum of dextran ~ 323.4ul
    #mastermix_rack
    LU_Mix = reagent_rack['D1'] # LU MasterMix; 14*96*1.20 = 1612.8ul; round up to 1650ul
    
    # USER INPUT
    dextran_beg_vol= 1000 # dextran volume in ul
    water_beg_vol = 1000 # water volume in ul

    # LISTS
    rows = ['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H']
    conds = [cond_1, cond_2, cond_3, cond_4, cond_5, cond_6, cond_7, cond_8, cond_9, cond_10, cond_11, cond_12]
    excess = 1.08
    # disp_locs = ['A1', 'A2', 'A3', 'A4', 'A5', 'A6', 'A7', 'A8', 'A9', 'A10', 'A11', 'A12'] #DISPENSE LOCATIONS ON PCR PLATE
    dextranVols =[0, 1, 1.5, 2, 2.5, 3, 3.5, 4, 4.5, 5, 5.5, 6] # dextran volumes in 20ul rxn. 4/20 at 25% = 5% dextran, current concentration in mastermixes
    waterVols= [20-14-vol for vol in dextranVols] #  
    print ("dextranVols: ", dextranVols)
    print ("waterVols: ", waterVols)

    # #### COMMANDS ######    
    # 1.1 Add 14*8*excess = 117.6ul of MasterMix to each condition tube on mix_rack
    # 1.2 Add Dextran*14*8*excess to condition tube on mix_rack
    # 1.3 Add [water-(14-Dextran)]*excess to condition tube on mix_rack
    # 1.4 mix condition tubes on alum block and aliquot to plate

    # 1.1 Add 14*8*excess = 117.6ul of LU MasterMix to condition tube on mix_rack
    h_list = tip_heights(1500, 12, 120.96) #96*14*1.10 = 1478.4ul; round up to 1500, 14*8*excess = 120.96
    p300.pick_up_tip()
    p300.mix(3, 200, LU_Mix.bottom(h_list[0]))
    for h, tube in enumerate(conds):
        # print ("tip height in tube is:", h_list[h])
        p300.aspirate(8*14*excess, LU_Mix.bottom(h_list[h]), rate=0.70) # 8*14*excess = 120.96ul
        p300.touch_tip(v_offset=-3, speed=30)
        p300.dispense(8*14*excess, tube.bottom(1))
        p300.blow_out(tube.bottom(8))
    p300.drop_tip()

    # 1.2 Add Dextran*14*8*excess to condition tube on mix_rack
    # multiply each dextran volume by 8*excess to get volume column
    dextranVolsTube = [vol*excess*8 for vol in dextranVols] # dextran volumes in 20ul rxn. 4/20 at 25% = 5% dextran, current concentration in mastermixes
    # print ("dextranVolsTube: ", dextranVolsTube)
    print ("sum(dextranVolsTube): ", sum(dextranVolsTube)) # total volume of dextran used for plate
    d_heights= tip_heights(dextran_beg_vol, 12, int(sum(dextranVolsTube)/len(dextranVolsTube)) ) # average dextran volume per tube
    print ("Dextran volume heights:", d_heights)
    for dVol, tube, h in zip(dextranVolsTube, conds, d_heights):
        if dVol < 0.01: # OT-2 logic needs this because if aspirate vol = 0 then default will be 20ul. 
            continue
        if dVol < 20: #for volumes < 3ul per well
            p20.pick_up_tip()
            p20.mix(1, 20, dextran.bottom(h), rate=0.5) #pre-moisten helps with accurate dispensing
            # print ("p20 pipette tip height in dextran addition to tubes is: ", h)
            p20.aspirate(dVol, dextran.bottom(h), rate=0.5)
            protocol.delay(seconds=2) #equilibrate tip
            p20.touch_tip(v_offset=-3, speed=30)
            p20.dispense(dVol, tube.bottom(4))
            protocol.delay(seconds=2) #equilibrate tip
            p20.blow_out(tube.bottom(12))
            p20.drop_tip()
        else: # for volumes > 3ul per well
            p300.pick_up_tip()
            p300.mix(1, 200, dextran.bottom(h), rate=0.5) #pre-moisten helps with accurate dispensing
            # print ("p20 pipette tip height in dextran addition to tubes is: ", h)
            p300.aspirate(dVol, dextran.bottom(h), rate=0.5)
            protocol.delay(seconds=2) #equilibrate tip
            p300.touch_tip(v_offset=-3, speed=30)
            p300.dispense(dVol, tube.bottom(4))
            protocol.delay(seconds=2) #equilibrate tip
            p300.blow_out(tube.bottom(12))
            p300.drop_tip()
    
    # 1.3 Add 2[0-(14-Dextran)]*excess NFW to condition tube on alum block
    waterVolsTube = [vol*8*excess for vol in waterVols] # water volumes in tubes
    # print ("waterVolsTube: ", waterVolsTube)
    w_heights= tip_heights(water_beg_vol, 12, int(sum(waterVolsTube)/len(waterVolsTube)) ) # average dextran volume per tube
    for wVol, tube, h in zip(waterVolsTube, conds, w_heights):
        if wVol < 0.01: # OT-2 logic needs this because if aspirate vol = 0 then default will be 20ul.
            continue
        if wVol < 20:
            p20.pick_up_tip()
            p20.mix(1, 20, water.bottom(h))
            # print ("p20 pipette tip height in water addition to tubes is: ", h)
            p20.aspirate(wVol, water.bottom(h))
            p20.dispense(wVol, tube.bottom(4))
            p20.blow_out(tube.bottom(8))
            p20.drop_tip()
        else: 
            p300.pick_up_tip()
            p300.mix(1, 200, water.bottom(h))
            p300.aspirate(wVol, water.bottom(h))
            p300.dispense(wVol, tube.bottom(4))
            p300.blow_out(tube.bottom(8))
            p300.drop_tip()

    # 1.4 mix condition tubes on alum block and aliquot to PCR plate
    p300.pick_up_tip()
    for col, tube in enumerate(conds): #col is a number, tube is a well
        tube_height = tip_heights(172.8, 8, 20) #20*8*1.08 = 172.8ul
        p20.pick_up_tip()
        p300.mix(4, 165, tube.bottom(1)) # tube total volume = 168
        protocol.delay(seconds=2) #equilibrate tip
        p300.blow_out(tube.bottom(10)) # don't want to add bubbles to tube
        p300.touch_tip(v_offset=-3, speed=30)
        for i, row in enumerate(rows):
            p20.aspirate(20, tube.bottom(tube_height[i]), rate=0.75)
            # print ("in 1.4, tip height is: ", tube_height[i])
            dest = row+str(col+1) 
            p20.dispense(20, plate[dest].bottom(2))
            p20.move_to(plate[dest].bottom(0)) # touch tip to remove droplets
        p20.drop_tip()
    p300.drop_tip()


    
