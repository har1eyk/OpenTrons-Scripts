...
# metadata
metadata = {
    'protocolName': 'Add 18ul Mix and 2ul Sample to Strip Cap Tubes--Test Melting Gradient',
    'author': 'Harley King <harley.king@luminultra.com>',
    'description': 'Creates strip cap tubes of optimal primer and probe concentrations to qPCR samples.',
    'apiLevel': '2.12'
}
##########################
# functions
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    # plate = tempdeck.load_labware('8wstriptubesonfilterracks_96_aluminumblock_250ul')
   ...
   # mix_rack
    ...
    #fuge_rack
    samp_1 = fuge_rack['A1'] # e.g. 0.625uM # empty
    samp_2 = fuge_rack['A2'] # e.g. 1.25uM # empty
    samp_3 = fuge_rack['A3'] # e.g. 2.5uM # empty
    samp_4 = fuge_rack['A4'] # e.g. 5.0uM # empty
    samp_5 = fuge_rack['A5'] # e.g. 7.5uM # empty
    samp_6 = fuge_rack['A6'] # e.g. 10uM # empty
    samp_7 = fuge_rack['B1'] # e.g. 10uM # empty
    ...
    # 1.1 add 18ul MMIX to 96 wells; 6 strp cap tubes on temp deck, 6 strip cap tubes in holder
    p20.pick_up_tip()
    tubeH = tip_heightsEpp(1800, 96, 18)
    h_counter = 0
    p20.mix(2, 20, mmix.bottom(tubeH[h_counter]))
    for row in rows:
        for platecol in range(0,12,2):
        ...
        for holdercol in range(0,12,2):
            ...
            p20.touch_tip(v_offset=-4, speed=40)
            # p20.move_to(mmix.bottom(tubeH[h_counter])) # remove fluid my dipping in mix
            destHolder = row + str(holdercol+1)
            p20.dispense(18, holder_1[destHolder].bottom(3))
            p20.blow_out(holder_1[destHolder].bottom(8))
            p20.move_to(holder_1[destHolder].bottom(3)) # remove traces of mmix
            h_counter +=1
    p20.drop_tip()
    ...
    for sample, row in zip(samp_sources, rows):
        for col in range(1):
            ...
            destTwo = row + str(col+3)
            destThree = row + str(col+5)
            destFour = row + str(col+7)
            destFive = row + str(col+9)
            destSix = row + str(col+11)
            ...
            p20.drop_tip()
        for holderCol in range(1):
            p20.pick_up_tip()
            p20.mix(1,20, sample.bottom(2)) # pre-wetting tip is more accurate
            p20.aspirate(18, sample.bottom(2))
            destHOne = row + str(holderCol+1)
            destHTwo = row + str(holderCol+3)
            destHThree = row + str(holderCol+5)
            destHFour = row + str(holderCol+7)
            destHFive = row + str(holderCol+9)
            destHSix = row + str(holderCol+11)
            p20.dispense(2, holder_1[destHOne].bottom(2))
            p20.dispense(2, holder_1[destHTwo].bottom(2))
            p20.dispense(2, holder_1[destHThree].bottom(2))
            p20.dispense(2, holder_1[destHFour].bottom(2))
            p20.dispense(2, holder_1[destHFive].bottom(2))
            p20.dispense(2, holder_1[destHSix].bottom(2))
            p20.drop_tip()
