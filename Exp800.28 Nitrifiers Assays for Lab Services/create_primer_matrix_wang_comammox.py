...
# metadata
metadata = {
    'protocolName': 'Create Primer Conc Matrix for Optimization for Wang COMAMMOX with EvaGreen.',
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    # plate = tempdeck.load_labware('amplifyt_96_aluminumblock_300ul') # keep this line because sometimes this plate is used.
    ...
    # REAGENTS
    # mix_rack
    BPW_mix = mmix_rack['A1'] # 14.8*96(1+0.3)=1847.04 (923.52*2)
    # sds_rack (keep this rack separate as to avoid contamination)
    std_1 = stds_rack['A1'] # 990ul Water and ssDNA standard
    std_2 = stds_rack['A2'] # 900ul water and ssDNA standard
    std_3 = stds_rack['A3'] # 900ul water and ssDNA standard
    std_4 = stds_rack['A4'] # 900ul water and ssDNA standard
    std_5 = stds_rack['A5'] # 900ul water and ssDNA standard
    std_6 = stds_rack['A6'] # 900ul water and ssDNA standard 
    std_7 = stds_rack['B1'] # 900ul water and ssDNA standard
    # sds_mix_rack
    sN_mix = stds_mix_rack['A1'] # empty; receives BPW_mix and water for stds 
    std_1mix = stds_mix_rack['C3'] # empty
    std_2mix = stds_mix_rack['C4'] # empty
    std_3mix = stds_mix_rack['C5'] # empty
    std_4mix = stds_mix_rack['C6'] # empty
    std_5mix = stds_mix_rack['D3'] # empty
    ...
    P_conc = 300 # EvaGreen: no probe, but preserving to keep vols correct 20230119 hjk
    ...
    print ("water_XFR_bpwd_mix", water_XFR_bpwd_mix)
    print (" F_800_int_primer", F_800_int_primer)
    ##### COMMANDS ######
    # prepare sN_mix
    ...
    R_height = (tip_heights(R_total_vol-R_add_to_sN_mix, len(R_mix_primer), 20)) # 300-16.92ul = 283.08ul
    for h, R_vol in enumerate(R_mix_primer):
        print("R_vol: ", R_vol)
        if R_vol <= 20:
            p20.pick_up_tip()
            p20.aspirate(R_vol, rev_10uM.bottom(R_height[h]))
            p20.dispense(R_vol, all_R_mix[h].bottom(8))
            p20.mix(2, R_vol, all_R_mix[h].bottom(8))
            p20.blow_out(all_R_mix[h].bottom(12))
            h =+ 1
            p20.drop_tip()
        else:
            p300.pick_up_tip()
            p300.aspirate(R_vol, rev_10uM.bottom(R_height[h]))
            p300.dispense(R_vol, all_R_mix[h].bottom(8))
            p300.mix(2, R_vol, all_R_mix[h].bottom(8))
            p300.blow_out(all_R_mix[h].bottom(12))
            h =+ 1
            p300.drop_tip()
    
    # add water
    w_height = (tip_heights(900, len(R_mix_water), 20)) # 
    for p, w_vol in enumerate(R_mix_water):
        print("w_vol: ", w_vol)
        if w_vol <= 20:
            p20.pick_up_tip()
            p20.aspirate(w_vol, water.bottom(w_height[p]))
            p20.dispense(w_vol, all_R_mix[p].bottom(8))
            p20.blow_out(all_R_mix[p].bottom(12))
            p =+ 1
            p20.drop_tip()
        else:
            p300.pick_up_tip()
            p300.aspirate(w_vol, water.bottom(w_height[p]))
            p300.dispense(w_vol, all_R_mix[p].bottom(8))
            p300.blow_out(all_R_mix[p].bottom(12))
            p =+ 1
            p300.drop_tip()
    ...
    for tube, row in zip(all_R_mix, plate_rows):
        p300.pick_up_tip()
        p300.mix(2, 200, tube.bottom(2)) # about 230ul in tube
        p300.mix(2, 200, tube.bottom(3)) # about 230ul in tube
        p300.mix(3, 200, tube.bottom(4)) # about 230ul in tube
        for i in range(0,2): # split 1..12 dispensing in half
            p300.aspirate(R_mix_rxn*2*3*1.05, tube.bottom(1), rate=0.4) # 18.4 *2 wells * 3 times on row*1.05 waste = 115.92ul
            ...
            for j in range(1+6*i,6+6*i,2): #1,3,5->7,9,11; distribute to every other col
                ...
                if j==5 or j==11:
                    ...
                    p300.dispense((1.05-1)*R_mix_rxn*2*3, tube.bottom(10))
                    p300.blow_out(tube.bottom(10))
                    # p300.touch_tip(liquid_trash, v_offset=-3)
                # p300.touch_tip()
        p300.drop_tip()
...
#  Mix 100ul 'Fwd' dilutions in tube by adding water and "fwd 10uM" tube as shown in schedule
    ...
    for i, tube in enumerate (all_fwd):
        ...
        p300.blow_out(tube.bottom(10))
        p300.move_to(tube.bottom(10))
        protocol.delay(seconds=2) #wait for droplets to coalesce
        p300.move_to(tube.bottom(3)) # remove droplets by touching fluid
        # p300.touch_tip() # don't like how this leaves fluid on tube side; try liquid touch instead
        p300.move_to(tube.bottom(40))
        ...
        for x in range(0, len(plate_rows)):  # distribute to every other col
            ...
            # p20.touch_tip() #don't like how fluid ends up on tube side
            p20.move_to(plate[well_pos].bottom(2))
        ...
        for y in range(0, len(plate_rows)):  # distribute to every other col
            ...
            p300.move_to(plate[well_pos].bottom(3)) # droplets removed in liquid
            ...
            p20.blow_out()
            #  p20.touch_tip() # leaeves droplet on side
            p20.move_to(plate[well_dest].bottom(2)) # remove droplets by touching fluid
        p300.drop_tip()
        p20.drop_tip()
