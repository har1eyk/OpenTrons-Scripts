...
def run(protocol: protocol_api.ProtocolContext):
    ...
    # lists
    detergent_vol = [1.83, 8.81, 16.82, 37, 52.86, 61.67, 69.38, 79.29, 87.63, 92.5, 123.33] # from excel spreadsheet
    lurb_vol = [183.17, 176.19, 168.18, 148, 132.14, 123.33, 115.63, 105.71, 97.37, 92.5, 61.67] # from excel spreadsheet
    dATP_vol = 15
...
# Add lurb
    ...
    for col in cols:
        if detergent_vol[count] <= 20: #if volume is under 20 uL, use p20
            ...
            sample_count = 0
            detergent_h = fifty_ml_heights(6750, 64, 85)
            p300.pick_up_tip()
            for row in rows:
                dest = row + str(cols[count])
            ...
            p300.drop_tip()
            count += 1 
        if count == len(cols)-1: # if last column, don't add. last col = water
            break
