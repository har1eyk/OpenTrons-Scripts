...
def run(protocol: protocol_api.ProtocolContext):
    ...
    mix_rack = protocol.load_labware('opentrons_15_tuberack_nest_15ml_conical', '5')    
    ...
    #mix_rack
    BPW_mix = fuge_rack['D1'] # see sheet, 4815ul
    ...
    rxn_base = 40.3 # Everything in PCR buffer (Mg2+, dNTPs, polymerase, enhancers, stabilizers, sugars, etc.). (in uL)
    tot_rxn_vol = 50 # What is total volume of PCR reaction in plate/tube? (in ul)
    ...
    sN_mix_waste_offset = 0.085  # How much percent_waste offset should sN_mix use? This calculated as percent_waste-sN_mix_overage = percent_waste for sN_mix_overage e.g. (20-7=13%) Should not be 0 otherwise offset = percent_waste. (decimal)
    ...
    std_NTC_waste_offset = 0.1 # How much percent_waste offset should std_NTC use? (decimal)
    ...
    rxn_vol_no_dna = tot_rxn_vol-dna_per_rxn_10x # Calc what is volume of PCR rxn with no DNA added. (in ul)
    ...
    std_woff_per_sN_rxn =  tot_rxn_vol-(BPW_rxn+std_vol_F_per_rxn+std_vol_R_per_rxn)# Calc how much water to add to offset diff in max vol F,R primer per reaction. (Max-Std)*2 (in ul)
    ...
    p300.pick_up_tip()
    bpw_h = fifteen_ml_heights(BPW_mix_tot, len(split_asp(BWP_mix_xfer_sN_mix, p300_max_vol)), split_asp(BWP_mix_xfer_sN_mix, p300_max_vol)[0])
    p300.mix(3, 200, BPW_mix.bottom(bpw_h[0]))
    for j in range(len(split_asp(BWP_mix_xfer_sN_mix, p300_max_vol))):
        xfer_vol = split_asp(BWP_mix_xfer_sN_mix, p300_max_vol)[j]
        p300.aspirate(xfer_vol, BPW_mix.bottom(bpw_h[j]), rate=0.75)
        protocol.delay(seconds=1) #equilibrate
        sN_mix_h = tip_heights(xfer_vol+xfer_vol*j, 1, 0)[0] # tip height in receiving tube as a function of vols in bpw_h, don't want tip to be submerged as it may affect vol in sN_mix tube
        p300.dispense(xfer_vol, sN_mix.bottom(sN_mix_h+5))
        p300.blow_out(sN_mix.bottom(sN_mix_h+10)) # want to be above liquid level
    ...
    std_mix_h = tip_heights(sN_mix_tot, len(std_mixes), sN_mix_xfer_to_stds_mix)#[13,11,8,6,4,2,0] #1284.48ul
    p300.pick_up_tip()
    # need to mix F, R and water
    p300.mix(2, 200, sN_mix.bottom(4)) #4mm from bottom; this algo better for mixing, #1284.48ul
    p300.mix(3, 200, sN_mix.bottom(8)) #8mm from bottom
    p300.mix(5, 200, sN_mix.bottom(std_mix_h[0])) #10mm from bottom
    for tube, h in zip(std_mixes, std_mix_h):
        p300.aspirate(sN_mix_xfer_to_stds_mix, sN_mix.bottom(h), rate=0.75) #158ul. 
        ...
        p300.touch_tip(v_offset=-3)
        p300.dispense(sN_mix_xfer_to_stds_mix, tube, rate=0.75)
    p300.drop_tip()
    ...
    for std, intTube, well in zip(std_tubes, std_mixes, std_wells):
        p20.pick_up_tip()
        p300.pick_up_tip()
        p20.aspirate(std_DNA_xfer_to_stds_mix, std.bottom(3)) #aspirate from std tube into into std_mix (intermediate tube) e.g. 6.6 ul
        p20.touch_tip()
        p20.dispense(std_DNA_xfer_to_stds_mix, intTube.bottom(2)) #added 6.6 into 158.4ul
    ...
    # create bpwd_mix 
     # First, add std_5 DNA or other std. 0.2ul added instead of 2ul to avoid using large volumes of std
    p20.pick_up_tip()
    p20.aspirate(dna_XFR_bpwd_mix, std_3.bottom(3))
    p20.dispense(dna_XFR_bpwd_mix, bpwd_mix.bottom())
