...
# metadata
metadata = {
    'protocolName': 'Distribute Mmix and Wastewater Samples from a single column to a 96w Plate.',
    'author': 'Harley King <harley.king@luminultra.com>',
    'description': 'Aliquoting SARS-CoV-2 mix to half plate and then adding 5ul sample extraction from one DeepWell plate.',
    'apiLevel': '2.11'
}
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    # #### COMMANDS ######    
    # aspirate mmix to all wells in 96w plate; 15*48 = 720*1.2=864
    h_list = tip_heightsEpp(864, 50, 15)
    well_num = 1
    p20.pick_up_tip()
    for row in rows: #8 rows
        for col in range(1,7): #12 cols
            dest = row+str(col)
