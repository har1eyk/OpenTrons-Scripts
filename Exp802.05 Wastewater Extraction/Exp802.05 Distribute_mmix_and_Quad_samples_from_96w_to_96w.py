...
# metadata
metadata = {
    'protocolName': 'Distribute Mmix and Wastewater Quandruplicated Samples (A1, A4, A7, A10) from 96w Deepwell to 96w Plate.',
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    # 24 samples on BioER plate are quadruplicated
    # so wells A1, A4, A7, A10 are the same sample (tech replicates)
    # use same tip for tech replicates (per 4)
    for x, ww_plate in enumerate(tot_ww_plates):
        for s in range(1,4): # 1..3; 3 cols with 24 samples 
            for row in rows:
                p20.pick_up_tip()
                for r in range(4): # 1..4; 4 technical replicates
                    source = row + str(s+3*(r)) #A1
                    dest = row + str(s+3*(r)) #A1
