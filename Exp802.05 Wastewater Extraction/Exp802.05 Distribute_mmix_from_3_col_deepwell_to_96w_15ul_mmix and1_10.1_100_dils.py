...
# metadata
metadata = {
    'protocolName': 'Distribute Mmix and Wastewater Samples from 3 Col on 2 Deepwell to 96w Plate. 2 reps plus dilution series.',
    'author': 'Harley King <harley.king@luminultra.com>',
    'description': 'Aliquoting SARS-CoV-2 mix to PCR plate and adding 5ul sample extraction from 3 col on 2 deepwell plates after 15ul mix.',
    'apiLevel': '2.12'
}
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    dil_plate = protocol.load_labware('biorad_96_wellplate_200ul_pcr', '4')
    ...
    water = water_rack['A1'] # 15mL conical tube
    ...
    #### COMMANDS ######    
    # aspirate mmix to all wells in 96w plate; 15*96 = 1.1=1728
    ...
    # add water for dilution series, 1:10
    w_list = fifteen_ml_heights(10000, 24, 90)# 6mL water; 90*24 = 2160, 200*24 = 3840 | 3840+2160 = 5120
    p300.pick_up_tip()
    for i in range(1,4): #three columns
        for j in range(0,7,2): #four rows per column, skip every other row
            for k in range(1,2): # groups of two
                dest = rows[j]+str(i) # A1 | C1
                dest2= rows[j+1]+str(i) # B1 | D1
                p300.aspirate(200, water.bottom(w_list[j*k]))
                p300.dispense(90, dil_plate[dest].bottom(1))
                p300.dispense(90, dil_plate[dest2].bottom(1))
                p300.dispense(20, water.bottom(w_list[j*k]))

    # add water for dilution series, 1:100
    for m in range (1,4): #three columns
        for n, row in enumerate(rows):
            dest = row+str(i)
            p300.aspirate(200, water.bottom(w_list[m*n]))
            p300.dispense(198, dil_plate[dest].bottom(1))
            p300.dispense(2, water.bottom(w_list[m*n]))
    p300.drop_tip()

    # add samples to dilution plate, mix and add to PCR plate
    p300.pick_up_tip()
    p20.pick_up_tip()
    for o, row in enumerate(rows):

# # from two plates, two columns in one plate (cols5, 11), one column from the other (col5); four replicates
...
#                 dest1 = row + str(8*x+4*col+1) #A1, #A2, #A3
#                 dest2 = row + str(8*x+4*col+2)
#                 dest3 = row + str(8*x+4*col+3)
#                 dest4 = row + str(8*x+4*col+4)
